...
ll n, m;        // dimension of the input matrix
char mat[105][105];
ll dp[105][105][105];
...
ll f(ll i, ll j, ll x ,ll y){
    ...
    // adding to result if the currect place is interesting place
    result += (mat[i][j] == '*');
    result += (mat[x][y] == '*');
...
int main(){
    ...
    cin>>t;
    while(t--){
        cin>>m>>n;              // first column and then row is taken in input
